...
/// @title USDCVaultPSMV2
/// @notice One-way Peg Stability Module: users deposit USDC and receive MAI (no reverse direction).
///         V2 of USDCVaultPSM. Strict superset of V1 plus an explicit `close()` finality
///         primitive distinct from BeefyVaultPSMV2's `evacuate` (emergency exit).
/// @dev `close()` is permanent and irreversible. The deployed `owner` MUST be a multisig
///      with quorum >= 2 — never a bare EOA — because a single-key compromise can permanently
///      brick the PSM with no recovery path. This is a deployment-time governance constraint,
///      not a contract-enforced check. Operational guidance for closure (mandatory pause-first,
///      atomic Safe MultiSend) lives in the close-runbook published with this contract.
contract USDCVaultPSMV2 {
  ...
  // V2 close-finality state — write-once, irreversible.
  uint256 public closedAt;
  bytes32 public closedReason;
  ...
  // V2-only errors. PSMClosed is the typed permanent-finality revert; AlreadyClosed
  // enforces close()'s write-once semantic.
  error PSMClosed(uint256 closedAt);
  error AlreadyClosed();
  ...
  // V2-only event. Emitted exactly once, on the call to `close()`.
  ...
  /// @notice Reverts with `PSMClosed(closedAt)` once `close()` has been called.
  ///         Independent from `pausable` — operational pauses (per-selector, two-way)
  ///         remain a separate orthogonal concern.
  modifier whenNotClosed() {
    if (closedAt != 0) revert PSMClosed(closedAt);
    _;
  }
  ...
  /// @dev Modifier order: `whenNotClosed` runs BEFORE `pausable`. Once closed, deposits
  ///      revert with `PSMClosed(closedAt)` (a permanent-looking error) regardless of
  ///      pause state. This is load-bearing for integrators and indexers reading the
  ///      revert as a finality signal.
  function deposit(
    uint256 _amount
  ) external whenNotClosed pausable {
  ...
  /// @notice Permanently retire this PSM. Sets `closedAt` to the current block timestamp,
  ///         records `_reason` for off-chain audit trail, and emits `Closed`. After this,
  ///         `deposit`, `setUpgrade`, and `cancelUpgrade` revert with `PSMClosed`.
  ///         Owner-callable cleanup (`withdrawMAI`, `claimFees`, `transferToken`) remains
  ///         reachable so governance can sweep residual funds.
  /// @dev Write-once. `_reason` is documentation-only metadata — indexers MUST NOT switch
  ///      on `closedReason` because the vocabulary is unbounded and reviewer-enforced.
  ///      For machine-readable retirement classification, derive from off-chain governance
  ///      metadata (the QIP, the multisig attestation post). See close-runbook.
  /// @param _reason A short bytes32 label, e.g. bytes32("low-tvl-sunset").
  function close(
    bytes32 _reason
  ) external onlyOwner {
    if (closedAt != 0) revert AlreadyClosed();
    closedAt = block.timestamp;
    closedReason = _reason;
    emit Closed(block.timestamp, _reason);
  }
  ...
  /// @dev Gated by `whenNotClosed` (R8): a closed PSM cannot exit the upgrade flow,
  ///      preventing emission of upgrade-cancel events on a contract presumed dead
  ///      by indexers and integrators.
  function cancelUpgrade() external onlyOwner whenNotClosed {
    stopped = false;
    upgradeTime = 0;
  }

  /// @dev Gated by `whenNotClosed` (R8): a closed PSM cannot enter the upgrade flow.
  ///      This blocks two failure modes: (1) post-close `setUpgrade()` emits state
  ///      transitions that look like liveness to indexers, and (2) post-close
  ///      `setUpgrade()` would reopen the post-timelock USDC sweep window via
  ///      `transferToken(USDC, …)`.
  function setUpgrade() external onlyOwner whenNotClosed {
    if (!stopped) {
      stopped = true;
