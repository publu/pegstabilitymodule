...
/// @title AaveUSDPSMV1
/// @notice PSM that holds a raw Aave V3 aToken as its yield-bearing reserve.
///         Carries the V2 safety surface — evacuate/sweep/claimRefund/forceSettle,
///         multi-guardian, configurable MAI, minimum reserves — while dropping
///         share math because the aToken's balance is already denominated in
///         the underlying asset (rebasing 1:1).
/// @dev V1 of AaveUSDPSM. Greenfield contract — no prior deployment to preserve.
///      `gem` is the aToken; `pool` and `underlying` are derived from the aToken
///      at initialization via `POOL()` and `UNDERLYING_ASSET_ADDRESS()`.
contract AaveUSDPSMV1 {
  uint256 public constant MAX_INT =
    115_792_089_237_316_195_423_570_985_008_687_907_853_269_984_665_640_564_039_457_584_007_913_129_639_935;

  // Configurable MAI address (not hardcoded — chain-agnostic).
  ...
  address public gem; // the aToken
  address public pool; // the Aave V3 Pool derived from gem.POOL()
  ...
  /// @notice Initialize the PSM. Derives the Aave pool + underlying from the aToken.
  /// @param _gem The Aave V3 aToken address (acts as the yield-bearing reserve)
  ...
  function initialize(
    ...
    address _pool = _aToken.POOL();

    maxDeposit = 1e24; // 1 million ether-equivalent — scaled by decimalDifference
    maxWithdraw = 1e24;
    underlying = _underlying;
    // MAI is 18 decimals on every chain; underlying (USDC-class) is 6.
    // Derive dynamically so we don't hardcode either side.
    decimalDifference = uint256(IERC20(_maiAddress).decimals() - IERC20(_underlying).decimals());
    gem = _gem;
    pool = _pool;
  ...
  /// @notice Re-approve the pool to spend unlimited underlying on the PSM's behalf.
  function approveGem() public {
    IERC20(underlying).approve(pool, MAX_INT);
  }
  ...
  function deposit(
    ...
    // Supply the full contract balance (including the fee) to the Aave pool.
    uint256 _supplyAmount = IERC20(underlying).balanceOf(address(this));
    IAavePool(pool).supply(underlying, _supplyAmount, address(this), 0);
  ...
  function scheduleWithdraw(
    ...
    if (withdrawalEpoch[msg.sender] != 0) revert WithdrawalAlreadyScheduled();
    ...
    if (_availableLiquidity < minimumReserves + _toWithdraw) revert MinimumReservesBreached();
  ...
  function withdraw() external pausable {
    ...
    if (_toWithdraw > totalStableLiquidity) revert NotEnoughLiquidity();

    // Pool sends net USDC directly to user; the fee portion stays as aToken
    // inside the PSM and accrues as claimable yield (aToken.balanceOf - totalStableLiquidity).
    IAavePool(pool).withdraw(underlying, _toWithdrawwFee, msg.sender);
  ...
  /// @notice Returns the amount available for new withdrawal scheduling (underlying decimals)
  function availableForWithdrawal() public view returns (uint256) {
    uint256 _availableLiquidity = totalStableLiquidity - totalQueuedLiquidity;
    if (_availableLiquidity <= minimumReserves) return 0;
    uint256 _available = _availableLiquidity - minimumReserves;
    uint256 _fee = calculateFee(_available, false);
    if (_available <= _fee) return 0;
    return _available;
  }
  ...
  /// @notice Owner can claim accumulated fees (rebasing yield above liabilities)
  function claimFees() external onlyOwner {
    if (evacuated) return;
    uint256 _stored = IAToken(gem).balanceOf(address(this));
    if (_stored > totalStableLiquidity) {
      uint256 _fees = _stored - totalStableLiquidity;
      IAavePool(pool).withdraw(underlying, _fees, msg.sender);
  ...
  /// @notice Emergency evacuation: withdraws all assets from the Aave pool and freezes the contract
  function evacuateVault() external onlyGuardianOrOwner {
    ...
    uint256 _preBalance = IAToken(gem).balanceOf(address(this));
    if (_preBalance > 0) {
      try IAavePool(pool).withdraw(underlying, type(uint256).max, address(this)) returns (uint256) {}
      catch (bytes memory reason) {
        emit RedeemFailed(reason);
      }
    }

    emit VaultEvacuated(msg.sender, _preBalance);
  }

  /// @notice Users with pending scheduled withdrawals recover their MAI after evacuation
  ...
  /// @notice Deposits idle USDC on the PSM into the Aave pool with correct bookkeeping
  function sweep() external onlyOwner {
    ...
    IAavePool(pool).supply(underlying, balance, address(this), 0);
  ...
  /// @notice Owner escape hatch: transfer arbitrary tokens out of the PSM.
  /// @dev Matches V2's protection logic — `gem` (the aToken) is protected
  ///      pre-evacuation, and `underlying` is additionally protected
  ///      post-evacuation until the 2-day upgrade window elapses.
